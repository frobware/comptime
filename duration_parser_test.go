...
func TestSyntaxError_Error(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.input, func(t *testing.T) {
			...
			syntaxErr := err.(*comptime.SyntaxError)
...
func TestOverflowError_Error(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run("", func(t *testing.T) {
			_, err := comptime.ParseDuration(tc.input, comptime.Microsecond, comptime.ParseModeMultiUnit, comptime.NoRangeChecking)
			if !errors.Is(err, &comptime.OverflowError{}) {
				t.Errorf("expected OverflowError, got %T", err)
				return
			}

			overflowErr := err.(*comptime.OverflowError)
...
func TestParseDurationOverflow(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.input, func(t *testing.T) {
			duration, err := comptime.ParseDuration(tc.input, comptime.Microsecond, comptime.ParseModeMultiUnit, comptime.NoRangeChecking)
...
func TestParseDurationSyntaxErrors(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			duration, err := comptime.ParseDuration(tc.input, comptime.Millisecond, comptime.ParseModeMultiUnit, comptime.NoRangeChecking)
...
func TestParseDurationRangeErrors(t *testing.T) {
	tests := []struct {
		...
		duration         time.Duration
		inRangeChecker   comptime.RangeChecker
	}{{
		description:    "empty string",
		input:          "",
		expectErr:      false,
	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			duration, err := comptime.ParseDuration(tc.input, comptime.Millisecond, comptime.ParseModeMultiUnit, tc.inRangeChecker)
			...
			if tc.expectErr {
				...
				rangeErr := err.(*comptime.RangeError)
